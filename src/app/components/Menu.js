...
function Menu({ onClose, onAddLocation, initialCoordinates, onRequestMapClick }) {
        ...
        >
          ...
          {(hoverRating || formData.rating) > i && (
            <div className="absolute top-0 left-0 overflow-hidden" style={{ 
              width: (hoverRating || formData.rating) >= i + 1 ? '100%' : '50%'
            }}>
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-gray-500/30 flex items-center justify-center z-50 p-4">
      <div className="w-full max-w-lg bg-white p-4 md:p-6 rounded-lg shadow-lg overflow-y-auto max-h-[90vh] z-10 text-black">
